...
namespace WSAdmin
{
    public class Program
    {
        ...
        {
            _givenName = "Admin" + RandomString(4);
            ...
            if (Secure)
            {
                using (var wss = new WebSocket("wss://localhost:4649/SAPI?name=" + _givenName))
                ...
                //using (var ws = new WebSocket ("wss://localhost:5963/Chat?name=nobita"))
                {
                    ...
                    wss.SetCookie(new Cookie("roles", "\"controller, gunfighter\""));
            ...
            {
                using (var wss = new WebSocket("ws://localhost:4649/SAPI?name=" + _givenName))
                //using (var ws = new WebSocket ("wss://localhost:5963/Echo"))
                //using (var ws = new WebSocket ("ws://localhost:4649/Chat"))
                //using (var ws = new WebSocket ("wss://localhost:5963/Chat"))
                //using (var ws = new WebSocket ("ws://localhost:4649/Chat?name=nobita"))
